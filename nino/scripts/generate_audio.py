...
Usage:
    ...
    python3 scripts/generate_audio.py --lang en    # English (v2)
...
def main():
    ap = argparse.ArgumentParser(description="Generate word MP3s via Gemini TTS.")
    ...
    for it in items:
        ...
        try:
            pcm = synthesize(text, args.lang, args.voice, api_key)
            pcm_to_mp3(pcm, dest)
        except urllib.error.HTTPError as e:
    ...
    print(f"\ndone: {made} generated, {skipped} skipped  (voice: {args.voice}, model: {_working_model or MODELS[0]})  ->  {outdir}")
